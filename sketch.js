...
function draw() {
	...
	if (gamestate === "shop") {
		...
		textSize(15)
		text("Price: " + macePrice, 100, 120)
		text("(Press I)", 100, 140)
		text("Amount purchaced: " + maceUpgrade + "/5", 100, 160)
		textSize(20)
		text('Upgrade Movement Speed', 100, 220)
		textSize(15)
		text("Price: " + speedPrice, 100, 240)
		text("(Press O)", 100, 260)
		text("Amount purchased: " + speedUp + "/5", 100, 280)
		if (keyWentDown("I") && maceUpgrade <= 4 && money >= 500 + 150 * maceUpgrade) {
			money -= macePrice
			sword.height += 10
			sword.width += 10
			maceUpgrade += 1
		...
		if (keyWentDown("O") && speedUp <= 4 && money >= 750 + 200 * speedUp) {
			money -= speedPrice
			playerspeed += 1
			speedUp += 1
			speedPrice += 200
